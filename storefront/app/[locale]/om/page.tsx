...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: l === 'sv' ? 'Om EMOTO' : 'About EMOTO' }
}
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <PageShell
      ...
      breadcrumbs={[{ name: 'EMOTO', href: `/${l}` }, { name: sv ? 'Om' : 'About' }]}
    >
      ...
      <Prose>
        <p>{sv ? 'EMOTO är en oberoende, svenskgrundad återförsäljare av högpresterande elektriska off-road- och gaturegistrerade motorcyklar. Vi säljer Sur-Ron idag och är byggda för att bära fler märken imorgon.' : 'EMOTO is an independent, Sweden-founded retailer of high-performance electric off-road and street-legal motorcycles. We sell Sur-Ron today and are built to carry more marques tomorrow.'}</p>
        ...
        <p>{sv ? 'EMOTO är en fristående butik. Sur-Ron® är ett varumärke som tillhör sin ägare. Vi imiterar inte märket — vi bygger en egen, ägbar retailerupplevelse runt produkterna.' : 'EMOTO is a standalone store. Sur-Ron® is a trademark of its owner. We do not imitate the marque — we build our own ownable retail experience around the products.'}</p>
      </Prose>
    </PageShell>
  )
}
